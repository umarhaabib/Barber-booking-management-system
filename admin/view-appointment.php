...
if(isset($_POST['submit']))
  {
      ...
      $remark=$_POST['remark'];
      $status=$_POST['status'];
      if(($_SESSION['availableslots']==0||$_SESSION['availableslots']<0) && $status=="Accepted")
      {
        echo "<script>
        alert('You can not select this appointment because you have no available slots, please add slots first')
        window.location.href='set-slots.php';
        ;</script>";
      }
      else
      {
   $query=mysqli_query($con, "update  tblbook set Remark='$remark',Status='$status' where ID='$cid'");
    ...
    sendMailTOUser($remark,$status,$_SESSION['appointmentuseremail']);
    echo '<script>alert("All remark has been updated. A mail has been sent to the user.")</script>';
...
<html>
<head>
<title> Barber || View Appointment </title>
...
<tr>
    <th>Email</th>
    <td><?php  echo $_SESSION['appointmentuseremail']=$row['Email'];?></td>
  </tr>
   <tr>
    <th>Mobile Number</th>
